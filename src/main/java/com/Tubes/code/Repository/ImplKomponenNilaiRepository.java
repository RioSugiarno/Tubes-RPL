...
public class ImplKomponenNilaiRepository implements KomponenNilaiRepository{
    ...
    @Override
    public BigDecimal findBobotByIdNilai(int idNilai) {
        String sql = "SELECT bobot FROM KomponenNilai WHERE ID_Nilai = ?";
        return jdbcTemplate.queryForObject(sql, new Object[]{idNilai}, BigDecimal.class);
    }

    @Override
    public String findKomponenById(int idNilai) {
        String sql = "SELECT Komponen FROM KomponenNilai WHERE ID_Nilai = ?";
        return jdbcTemplate.queryForObject(sql, new Object[]{idNilai}, String.class);
    }
}

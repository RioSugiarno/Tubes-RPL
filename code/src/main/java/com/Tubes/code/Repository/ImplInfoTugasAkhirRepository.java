...
public class ImplInfoTugasAkhirRepository implements InfoTugasAkhirRepository {
    ...
    @Override
    public int findIdTaByNim(String nim) {
        String sql = "SELECT id_ta FROM InformasiTugasAkhir WHERE nim = ? LIMIT 1";
        return jdbcTemplate.queryForObject(sql, new Object[]{nim}, Integer.class);
    }

    @Override
    public String findNamaDosenByNID(String nid) {
        String query = "SELECT nama FROM Dosen WHERE nid = ? LIMIT 1";
        return jdbcTemplate.queryForObject(query, String.class, nid);
    }
}
